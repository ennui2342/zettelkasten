...
_STEP2_EDIT = (
    ...
    "{targets}\n"
    "\n"
    "Draft note (context only — do not add its content to the rewritten note):\n"
    "{draft}\n"
    "\n"
    "The existing note has grown large through repeated updates. Rewrite it to be "
    "tighter and more concise. Remove redundancy, repeated framings of the same "
    "point, and over-elaboration. Preserve all essential claims, distinctions, and "
    "examples — the goal is a denser note on the same topic, not a summary. The "
    "draft is provided as context to indicate which aspects of the note are most "
    "currently relevant and should be preserved with priority.\n"
    "\n"
...
_STEP2_SPLIT = (
    ...
    "{targets}\n"
    "\n"
    "Draft note (clarifies that two distinct topics are conflated):\n"
    "{draft}\n"
    "\n"
    "Rewrite the existing note as TWO separate topic notes. Each should be "
    "coherent and self-contained. The first note retains the original note's "
    "identity; the second is the newly separated topic.\n"
    "\n"
    f"{_NO_SOURCE_REFS}\n"
    "\n"
